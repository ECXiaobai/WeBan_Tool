...
class WeBanClient:
    ...
    def __init__(
        ...
        :param jupiter_fallback: 对未加载 apicenext.js 的课程也补发 jupiter
            ...
            个别学校可能要求该校所有微课都有轨迹，
            实测无轨迹会 10018 时可开启该项
        :param course_captcha_mode: 课程完课验证码处理方式：
            "api"     = 纯请求固定坐标点选破解（不启动浏览器），源自
                        WeibanCourseHelper 技巧，最快；
            "browser" = 启动无头/可见浏览器自动识别，失败转手动
                        （原 WeBan 行为）；
            "auto"    = 先尝试 api 破解，失败/校验未过再回退浏览器（默认）
        """
        self.log = log
        ...
        if course_captcha_mode not in ("api", "browser", "auto"):
            raise ValueError(f"未知的 course_captcha_mode: {course_captcha_mode}")
        self.course_captcha_mode = course_captcha_mode
...
jupiter_fallback=true 时也补翻页轨迹。再答题，最后完课。
    ...
    def _resolve_course_captcha_token(
        self,
        course: dict,
        task: dict,
        course_url: str,
    ) -> str | None:
        """获取课程完课验证码 token（合并自 WeibanCourseHelper 的轻量技巧）。

        按 course_captcha_mode 决定策略：
        - "api"：仅用纯请求固定坐标点选破解（getCaptcha/checkCaptcha，不启动浏览器），
          失败返回 None；
        - "browser"：仅用浏览器自动化识别（原 WeBan 行为），失败返回 None；
        - "auto"（默认）：先尝试 api 破解，失败再回退浏览器，都失败返回 None。
        :return: 完课 token；None 表示失败（调用方按完课失败处理，继续下一门课）
        """
        mode = self.course_captcha_mode
        # 阶段一：纯请求固定坐标点选破解（WeibanCourseHelper 技巧，无需浏览器）
        if mode in ("api", "auto"):
            try:
                self.log.info("课程验证码：尝试轻量请求破解（不启动浏览器）")
                invoke_res = self.api.invoke_captcha(
                    course["userCourseId"], task["userProjectId"]
                )
                if _check_code_ok(invoke_res):
                    token = (invoke_res.get("data") or {}).get("methodToken") or ""
                    if token:
                        self.log.success("课程验证码轻量破解成功")
                        return token
                    self.log.warning("轻量破解未返回 methodToken，回退下一策略")
                else:
                    self.log.warning(f"轻量破解失败：{invoke_res}")
            except PermissionError:
                raise  # Token 失效，立即终止该账号
            except Exception as e:  # noqa: BLE001 -- 网络异常降级到下一策略
                self.log.warning(f"轻量破解异常: {e}")
            if mode == "api":
                return None
        # 阶段二：浏览器自动化识别（原 WeBan 行为）
    ...
    def _finish_course(
        ...
        if query.get("csCapt", [None])[0] == "true":
            try:
                captcha_token = self._resolve_course_captcha_token(
                    course, task, course_url
                )
            except PermissionError:
                raise  # Token 失效，立即终止该账号
            if not captcha_token:
                return {"code": "-1"}
            finish_kwargs["token"] = captcha_token
