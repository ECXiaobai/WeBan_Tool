...
# 远程模板下载地址：离线分发版留空，直接使用打包内置的 config.example.toml
CONFIG_EXAMPLE_URL = ""
...
def run_account(
    ...
    course_captcha_mode = get_setting("course_captcha_mode", "auto")
    ...
    try:
        ...
        if token_val and user_id:
            ...
            client = WeBanClient(
                ...
                course_captcha_mode=course_captcha_mode,
            )
...
if __name__ == "__main__":
    try:
        ...
        _check_update_async()  # 异步检查更新（离线分发版为空仓库，直接跳过）
